...
public class LifeItem extends Item {
    ...
    public LifeItem(GridPosition position) {
        this.visible = false;
        this.blocked = false;
        this.destroyable = true;
        this.entityType = EntityType.life_item;
    ...
    public void updateGameState(GameState gameState) throws CloneNotSupportedException {
        ...
        for(Entity e : gameState.getEntityList()) {
            if(e instanceof PlayerAgent && e.getPosition().distance(this.getPosition()) == 0) {
                ((PlayerAgent) e).addExtraLife();
