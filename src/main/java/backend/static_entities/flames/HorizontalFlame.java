...
public class HorizontalFlame extends Flame {
    ...
    public void updateGameState(GameState gameState) {
        ...
        for(Entity e : gameState.getEntityList()) {
            if(e.isDestroyable() && e.isVisible() && this.getPosition().distance(e.getPosition()) <= 1 && this.getPosition().getY() == e.getPosition().getY()) {
                e.destroy();
            }
            else if(e instanceof PlayerAgent && this.getPosition().distance(e.getPosition()) <= 1.3 && (Math.abs(this.getPosition().getY() - e.getPosition().getY()) < 0.3)) {
                e.destroy();
            }
        }
    }
